...
public class Worker {
    ...
    private static final String EXCHANGE_NAME = "stos";
    private static final String TASK_ROUTING_KEY = "ev_files";
    private static final String LOGGER_ROUTING_KEY = "log";
    private static final String FILE_SERVER_ROUTING_KEY = "file_server";
    ...
    public void sendTaskMessage(byte[] message) {
        rabbitTemplate.convertAndSend(EXCHANGE_NAME, TASK_ROUTING_KEY, message);
        System.out.println("Task message sent.");
    }

    public void sendLogs(byte[] logs){
        rabbitTemplate.convertAndSend(EXCHANGE_NAME, LOGGER_ROUTING_KEY, logs);
        System.out.println("Log sent.");
    }

    public void sendFiles(List<Messages.File> dataList){
        Messages.Files files = Messages.Files.newBuilder()
                .addAllContent(dataList)
                .setUuid("glowakota")
                .build();
        rabbitTemplate.convertAndSend(EXCHANGE_NAME, FILE_SERVER_ROUTING_KEY, files.toByteArray());
    }
